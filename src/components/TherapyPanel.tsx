...
export default function TherapyPanel({
  ...
  // Helper function to generate and download a gorgeous high-fidelity direct binary PDF report
  const downloadPdfReport = () => {
    const beforeBpm = activeData.heartRate;
    const afterBpm = Math.max(65, beforeBpm - 12);
    const beforeStress = activeData.stressLevel;
    const afterStress = Math.max(22, Math.round(beforeStress * 0.42));
    const beforeEnergy = activeData.energyScore;
    const afterEnergy = Math.min(96, beforeEnergy + 24);
    const beforeRes = activeData.breathingRate;
    const afterRes = 6; // Deep box breathing
    const dateStr = new Date().toLocaleDateString('tr-TR') + ' ' + new Date().toLocaleTimeString('tr-TR');
    
    // Turkish-to-Universal Latin mapper to ensure total compatibility with standard Helvetica in all PDF viewers
    const safeText = (txt: string): string => {
      if (!txt) return "";
    ...
    // 1. Elegant Page Borders & Corner Frames
    doc.setDrawColor(109, 40, 217); // Purple border
    doc.setLineWidth(0.4);
    doc.rect(8, 8, 194, 281); // elegant outer frame
    
    // Draw double lines at top
    doc.setDrawColor(13, 148, 136); // Teal accent
    doc.rect(9.5, 9.5, 191, 278);

    // 2. Document Title Header
    doc.setTextColor(109, 40, 217); // Deep Purple
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(16);
    doc.text(safeText("KUANTUM BIYO-REZONANS & MANEVI RE-ALIGNMENT"), 15, 20);
    
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.setTextColor(13, 148, 136); // Teal
    doc.text(safeText("RESMI DEGERLENDIRME VE DENGELEME SEANS RAPORU"), 15, 25);
    
    // Header Metadata (ID & Date) on right
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
    doc.setTextColor(100, 116, 139);
    doc.text(safeText(`RAPOR KODU: QR-${Math.floor(Math.random() * 900000 + 100000)}`), 142, 20);
    doc.text(safeText(`SEANS TARIHI: ${dateStr}`), 142, 25);
    ...
    // 3. Client & Operator Profile Section
    doc.setFillColor(248, 250, 252); // soft slate backing card
    doc.rect(15, 33, 180, 25, 'F');
    doc.setDrawColor(241, 245, 249);
    doc.rect(15, 33, 180, 25);

    doc.setFontSize(9);
    doc.setTextColor(71, 85, 105);
    doc.setFont('helvetica', 'normal');
    doc.text(safeText("Danisan Profili:"), 18, 39);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(30, 41, 59);
    doc.text(safeText(activeData.id === 'dummy' ? "Aktif Re-Kalibrasyon / Standart Profil Kaydi" : `Biyofotonik rPPG Profili [ID: ${activeData.id}]`), 46, 39);

    doc.setFont('helvetica', 'normal');
    doc.setTextColor(71, 85, 105);
    doc.text(safeText("Sorumlu Uzman:"), 18, 45);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(30, 41, 59);
    doc.text(safeText("Psikolog & Kuran Rezonansi Kurucusu Abdulkadir Kan"), 46, 45);

    doc.setFont('helvetica', 'normal');
    doc.setTextColor(71, 85, 105);
    doc.text(safeText("Engellenen Bolge:"), 18, 51);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(109, 40, 217); // Purple blockage
    doc.text(safeText(activeData.blockageLocation), 46, 51);

    // 4. Section 1: Biometric & Cellular Evaluation Table
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(15, 23, 42);
    doc.setFontSize(10.5);
    doc.text(safeText("1. HUCRESEL VE BIYOMETRIK VERI KARSI-ANALIZI"), 15, 66);

    // Table Header Row
    doc.setFillColor(109, 40, 217); // Deep Purple Backing for headers
    doc.rect(15, 71, 180, 8, 'F');
    doc.setFontSize(8.5);
    doc.setTextColor(255, 255, 255); // White text
    doc.setFont('helvetica', 'bold');
    doc.text(safeText("BIYOMETRIK DEGERLER"), 18, 76.5);
    doc.text(safeText("ONCESI (BAZAL EST.)"), 72, 76.5);
    doc.text(safeText("SONRASI (COHERENCE)"), 114, 76.5);
    doc.text(safeText("YENILENME ORANI"), 158, 76.5);
    ...
    // Row 1: Heart Rate
    doc.setFont('helvetica', 'normal');
    doc.text(safeText("Kalp Ritim Hizi (PPG Analizi)"), 18, 84);
    doc.text(safeText(`${beforeBpm} BPM`), 72, 84);
    doc.text(safeText(`${afterBpm} BPM`), 114, 84);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(13, 148, 136); // Teal for positive outcome
    doc.text(safeText(`-${beforeBpm - afterBpm} BPM (Parasempatik)`), 158, 84);
    drawRowLine(87);

    // Row 2: Stress Cortizol
    doc.setTextColor(30, 41, 59);
    doc.setFont('helvetica', 'normal');
    doc.text(safeText("Sikismis Ruhsal Stres Endeksi"), 18, 92);
    doc.text(safeText(`%${beforeStress}`), 72, 92);
    doc.text(safeText(`%${afterStress}`), 114, 92);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(13, 148, 136);
    doc.text(safeText(`-%${beforeStress - afterStress} Gerginlik Azalimi`), 158, 92);
    drawRowLine(95);

    // Row 3: Auric Aura Charge
    doc.setTextColor(30, 41, 59);
    doc.setFont('helvetica', 'normal');
    doc.text(safeText("Hucresel Vitalite (Aura Gucu)"), 18, 100);
    doc.text(safeText(`%${beforeEnergy}`), 72, 100);
    doc.text(safeText(`%${afterEnergy}`), 114, 100);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(13, 148, 136);
    doc.text(safeText(`+%${afterEnergy - beforeEnergy} Hücre Enerji Sarji`), 158, 100);
    drawRowLine(103);

    // Row 4: Respiration Frequency
    doc.setTextColor(30, 41, 59);
    doc.setFont('helvetica', 'normal');
    doc.text(safeText("Solunum Frekansi Genisligi"), 18, 108);
    doc.text(safeText(`${beforeRes} Nefes/dk (Sig)`), 72, 108);
    doc.text(safeText(`${afterRes} Nefes/dk (Kutucuk)`), 114, 108);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(13, 148, 136);
    doc.text(safeText("Frekans Senkronu Tam"), 158, 108);
    drawRowLine(111);

    // 5. Section 2: Matched Spiritual Quranic Remedy Detail
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(15, 23, 42);
    doc.setFontSize(10.5);
    doc.text(safeText("2. RECETELENDIRILEN MANEVI SIFA VE AKUSTIK REZONANS"), 15, 122);

    // Backing Card for Quran remedy details
    doc.setFillColor(245, 243, 255); // Soft purple
    doc.rect(15, 126, 180, 52, 'F');
    doc.setDrawColor(221, 214, 254);
    doc.rect(15, 126, 180, 52);

    doc.setFontSize(10);
    doc.setTextColor(109, 40, 217); // Purple for primary item
    doc.setFont('helvetica', 'bold');
    doc.text(safeText(`Ayet Akustik Rezonansi: ${targetSurah.name} (${targetSurah.verses}. Ayetler)`), 19, 132.5);

    doc.setFontSize(8.5);
    doc.setTextColor(71, 85, 105);
    doc.setFont('helvetica', 'normal');
    
    doc.text(safeText("Şifa Sinerjisi ve Belirti:"), 19, 139);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(30, 41, 59);
    doc.text(safeText(targetSurah.indication), 55, 139);

    doc.setFont('helvetica', 'normal');
    doc.setTextColor(71, 85, 105);
    doc.text(safeText("Tavsiye Olunan Esma-ül Hüsna:"), 19, 145);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(30, 41, 59);
    doc.text(safeText(targetSurah.esma), 65, 145);

    doc.setFont('helvetica', 'normal');
    doc.setTextColor(71, 85, 105);
    doc.text(safeText("Akustik Frekans Uyumlaması:"), 19, 151);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(13, 148, 136); // Teal for Hz
    doc.text(safeText(`${targetSurah.healingFrequencyHz} Hz Rezonans Dalgasi`), 65, 151);

    // Description italics
    doc.setFont('helvetica', 'italic');
    doc.setTextColor(109, 40, 217);
    doc.setFontSize(8);
    const textWrappedDesc = doc.splitTextToSize(safeText(targetSurah.description), 170);
    doc.text(textWrappedDesc, 19, 159);

    // 6. Section 3: Detailed Session Action Explanations
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(15, 23, 42);
    doc.setFontSize(10.5);
    doc.text(safeText("3. UYGULANAN SEANS ISLEMLERI VE REAL-TIME ANALIZ DETAYLARI"), 15, 189);

    // Let's explain thoroughly what was done step-by-step
    doc.setFillColor(254, 254, 255);
    doc.setDrawColor(226, 232, 240);
    doc.rect(15, 193, 180, 56, 'F');
    doc.rect(15, 193, 180, 56);

    doc.setFontSize(8.5);
    doc.setTextColor(30, 41, 59);

    // Bullet 1: Biometric Scanning
    doc.setFont('helvetica', 'bold');
    doc.text(safeText("A. Biyofotonik Optik rPPG Tarama Analizi:"), 19, 199);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(71, 85, 105);
    const p1Wrap = doc.splitTextToSize(safeText("Kamera objektifi uzerinden yuz meridyenlerindeki mikro kılcal damarlardaki kan hacmi degisimleri (photoplethysmography) anlık olarak sayısallastırılmıstır. Bu analiz sonucunda bazal kalp hızı, sempatik otonom stres katsayısı ve enerji tıkanıklıgı merkez noktası tespit edilmistir."), 170);
    doc.text(p1Wrap, 19, 203);

    // Bullet 2: Box Breathing
    doc.setTextColor(30, 41, 59);
    doc.setFont('helvetica', 'bold');
    doc.text(safeText("B. Coherence Kutucuk (Box) Nefes Re-Kalibrasyonu:"), 19, 215);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(71, 85, 105);
    const p2Wrap = doc.splitTextToSize(safeText("Seans suresince 4 saniye nefes alma, 4 saniye tutma (akciger dolu), 4 saniye verme ve 4 saniye bos tutma seklindeki dairesel solunum ritmiyle sempatik otonom sinir sistemi uyarısı baskılanmıs, kalbin ritmik senkronizasyonu coherence fazına basarıyla yukseltilmistir."), 170);
    doc.text(p2Wrap, 19, 219);

    // Bullet 3: Sound formulation
    doc.setTextColor(30, 41, 59);
    doc.setFont('helvetica', 'bold');
    doc.text(safeText("C. Akustik Solfeggio & Binaural Beat Ses Sentezi:"), 19, 231);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(71, 85, 105);
    const p3Wrap = doc.splitTextToSize(safeText("Kullanıcının kulaklık kanalları uzerinden sol kulaga tasıyıcı dalga, sag kulaga ise es zamanlı derin meditasyon dalgaları (Theta/Alpha aralıgı) binaural beat formulasyonu ile enjekte edilmistir. Bu sayede beyin dalgaları kaotik Beta seviyesinden şifa frekansı olan sakin tınılara kilitlenmistir."), 170);
    doc.text(p3Wrap, 19, 235);

    // 7. Official Certification Block
    doc.setFillColor(236, 253, 245); // Soft emerald backing
    ...
    doc.text(safeText("ENTEGRASYON ONAYI VE ALAN MUTABAKATI: ALIGNED & SECURED"), 19, 261);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(4, 120, 87);
    const certText = doc.splitTextToSize(safeText("Bu belge ile kullanicinin biyofotonik enerji katsayilarinin dengelendigi, elektromanyetik hucresel kalkaninin dengeli bir sekilde aktive edildigi teyit edilmistir. Seans basariyla tamamlanmistir ve otonom dengeleme oranlari guvence altina alinmistir."), 170);
    doc.text(certText, 19, 265.5);

    // Clean footer watermark
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7.5);
    doc.setTextColor(148, 163, 184);
    doc.text(safeText("Kuantum Biyo-Rezonans ve Ruhsal Re-notasyon Teknolojileri, Psikolog Abdulkadir Kan onay damgasi altindadir"), 15, 281);
    doc.setFont('helvetica', 'bold');
    doc.text(safeText("Manevi Rezonans ve Biyo-Gelisim Sistemleri - TS-24-2026"), 15, 285);

    // Trigger PDF File Download directly
    doc.save(`Kuantum_Sifa_Rezonans_Analizi_${activeData.id}.pdf`);
  };
